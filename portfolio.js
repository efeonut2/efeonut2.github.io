const portfolioItems = [
    {
        id: 6,
        title: "ALTINTEPE'DE FERAH BALKONLU ASANSÖRLÜ SATILIK 2+1 ARAKAT DAİRE",
        title_en: "SPACIOUS 2+1 MID-FLOOR APARTMENT FOR SALE WITH BALCONY & ELEVATOR IN ALTINTEPE",
        location: "Maltepe, Altıntepe Mh.",
        location_en: "Maltepe, Altintepe Dist.",
        price: "5.750.000 TL",
        type: "satilik",
        images: [
            "altintepe_altintepe (1).jpg",
            "altintepe_altintepe (2).jpg",
            "altintepe_altintepe (3).jpg",
            "altintepe_altintepe (4).jpg",
            "altintepe_altintepe (5).jpg",
            "altintepe_altintepe (6).jpg",
            "altintepe_altintepe (7).jpg",
            "altintepe_altintepe (8).jpg",
            "altintepe_altintepe (9).jpg",
            "altintepe_altintepe (10).jpg",
            "altintepe_altintepe (11).jpg",
            "altintepe_altintepe (12).jpg",
            "altintepe_altintepe (13).jpg",
            "altintepe_altintepe (14).jpg",
            "altintepe_altintepe (15).jpg",
            "altintepe_altintepe (16).jpg",
            "altintepe_altintepe (17).jpg",
            "altintepe_altintepe (18).jpg",
            "altintepe_altintepe (19).jpg",
            "altintepe_altintepe (20).jpg",
            "altintepe_altintepe (21).jpg",
            "altintepe_altintepe (22).jpg",
            "altintepe_altintepe (23).jpg",
            "altintepe_altintepe (24).jpg",
            "altintepe_altintepe (25).jpg",
            "altintepe_altintepe (26).jpg",
            "altintepe_altintepe (27).jpg",
            "altintepe_altintepe (28).jpg"
        ],
        coverImage: "altintepe_altintepe (1).jpg",
        rooms: "2+1",
        area: "85 m²",
        bath: "1",
        status: "Satılık",
        status_en: "For Sale",
        description: "Maltepe Altıntepe'de, Bağdat Caddesi ve E-5'e yakın konumda; güneybatı cepheli, ferah, aydınlık, balkonlu ve asansörlü 2+1 satılık arakat daire. Hem oturuma hem yatırıma uygun.",
        description_en: "Spacious and bright 2+1 mid-floor apartment for sale in Maltepe Altıntepe, close to Bağdat Avenue and the E-5 highway; southwest facade, with balcony and elevator. Suitable for both living and investment.",
        details: {
            "İlan Tarihi": "29 Haziran 2026",
            "Emlak Tipi": "Satılık Daire",
            "m² (Brüt)": "85",
            "m² (Net)": "70",
            "Oda Sayısı": "2+1",
            "Bina Yaşı": "26-30 arası",
            "Bulunduğu Kat": "2",
            "Kat Sayısı": "4",
            "Isıtma": "Kombi (Doğalgaz)",
            "Banyo Sayısı": "1",
            "Mutfak": "Kapalı",
            "Balkon": "Var",
            "Asansör": "Var",
            "Otopark": "Yok",
            "Eşyalı": "Hayır",
            "Kullanım Durumu": "Kiracılı",
            "Site İçerisinde": "Hayır",
            "Krediye Uygun": "Hayır",
            "Tapu Durumu": "Arsa Tapulu"
        },
        details_en: {
            "Listing Date": "June 29, 2026",
            "Property Type": "Apartment for Sale",
            "m² (Gross)": "85",
            "m² (Net)": "70",
            "Rooms": "2+1",
            "Building Age": "26-30",
            "Floor": "2",
            "Total Floors": "4",
            "Heating": "Natural Gas (Combi)",
            "Bathrooms": "1",
            "Kitchen": "Closed",
            "Balcony": "Yes",
            "Elevator": "Yes",
            "Parking": "No",
            "Furnished": "No",
            "Occupancy": "Tenanted",
            "In Complex": "No",
            "Credit Eligible": "No",
            "Title Deed": "Land Title"
        }
    },
    {
        id: 7,
        title: "PENDİK SÜLÜNTEPE'DE FERAH ASANSÖRLÜ ARAKAT 2+1 KİRALIK DAİRE",
        title_en: "SPACIOUS 2+1 MID-FLOOR APARTMENT FOR RENT WITH ELEVATOR IN PENDİK SÜLÜNTEPE",
        location: "Pendik, Sülüntepe Mh.",
        location_en: "Pendik, Sülüntepe Dist.",
        price: "30.000 TL",
        type: "kiralik",
        images: [
            "pendik_pendik (1).jpg",
            "pendik_pendik (2).jpg",
            "pendik_pendik (3).jpg",
            "pendik_pendik (4).jpg",
            "pendik_pendik (5).jpg",
            "pendik_pendik (6).jpg",
            "pendik_pendik (7).jpg",
            "pendik_pendik (8).jpg",
            "pendik_pendik (9).jpg",
            "pendik_pendik (10).jpg",
            "pendik_pendik (11).jpg",
            "pendik_pendik (12).jpg",
            "pendik_pendik (13).jpg",
            "pendik_pendik (14).jpg",
            "pendik_pendik (15).jpg",
            "pendik_pendik (16).jpg",
            "pendik_pendik (17).jpg",
            "pendik_pendik (18).jpg",
            "pendik_pendik (19).jpg",
            "pendik_pendik (20).jpg",
            "pendik_pendik (21).jpg",
            "pendik_pendik (22).jpg",
            "pendik_pendik (23).jpg",
            "pendik_pendik (24).jpg",
            "pendik_pendik (25).jpg",
            "pendik_pendik (26).jpg"
        ],
        coverImage: "pendik_pendik (1).jpg",
        rooms: "2+1",
        area: "85 m²",
        bath: "1",
        status: "Kiralık",
        status_en: "For Rent",
        description: "Pendik Sülüntepe'de, ana caddeye yalnızca bir sokak mesafede; gün boyu ışık alan aydınlık cepheli, açık balkon ve Fransız balkonlu, asansörlü 2+1 kiralık arakat daire. Açık otoparklı, bahçeli müstakil binada; sessiz ve sakin sokakta, Sabiha Gökçen Havalimanı'na yaklaşık 15 dakika. Aileler ve çalışanlar için ideal.",
        description_en: "Spacious and bright 2+1 mid-floor apartment for rent in Pendik Sülüntepe, just one street from the main avenue; sun-lit facade, with an open balcony, French balcony and elevator. Located in a detached, garden building with open parking, on a quiet street, about 15 minutes from Sabiha Gökçen Airport. Ideal for families and professionals.",
        details: {
            "İlan Tarihi": "30 Haziran 2026",
            "Emlak Tipi": "Kiralık Daire",
            "m² (Brüt)": "85",
            "m² (Net)": "70",
            "Oda Sayısı": "2+1",
            "Bina Yaşı": "11-15 arası",
            "Bulunduğu Kat": "1",
            "Kat Sayısı": "4",
            "Isıtma": "Kombi (Doğalgaz)",
            "Banyo Sayısı": "1",
            "Mutfak": "Kapalı",
            "Balkon": "Var",
            "Asansör": "Var",
            "Otopark": "Açık Otopark",
            "Eşyalı": "Hayır",
            "Kullanım Durumu": "Boş",
            "Site İçerisinde": "Hayır",
            "Aidat (TL)": "500",
            "Depozito (TL)": "30.000",
            "Tapu Durumu": "Kat Mülkiyetli"
        },
        details_en: {
            "Listing Date": "June 30, 2026",
            "Property Type": "Apartment for Rent",
            "m² (Gross)": "85",
            "m² (Net)": "70",
            "Rooms": "2+1",
            "Building Age": "11-15",
            "Floor": "1",
            "Total Floors": "4",
            "Heating": "Natural Gas (Combi)",
            "Bathrooms": "1",
            "Kitchen": "Closed",
            "Balcony": "Yes",
            "Elevator": "Yes",
            "Parking": "Open Parking",
            "Furnished": "No",
            "Occupancy": "Empty",
            "In Complex": "No",
            "Maintenance Fee (TL)": "500",
            "Deposit (TL)": "30.000",
            "Title Deed": "Condominium (Kat Mülkiyeti)"
        }
    }
];

const container = document.getElementById('portfolioContainer');
const searchInput = document.getElementById('searchInput');
const typeFilter = document.getElementById('typeFilter');
const searchBtn = document.getElementById('searchBtn');

// Modal Elements
const modal = document.getElementById('listingModal');
const closeModal = document.querySelector('.close-modal');
const modalMainImage = document.getElementById('modalMainImage');
const modalThumbnails = document.getElementById('modalThumbnails');
const modalTitle = document.getElementById('modalTitle');
const modalLocation = document.getElementById('modalLocation');
const modalPrice = document.getElementById('modalPrice');
const modalDetailsGrid = document.getElementById('modalDetailsGrid');
const modalDescription = document.getElementById('modalDescription');

// Check Language
const isEnglish = document.documentElement.lang === 'en';

function renderPortfolio(items) {
    container.innerHTML = '';

    // No results message
    const noResultsTitle = isEnglish ? "No results found." : "Sonuç bulunamadı.";
    const noResultsDesc = isEnglish ? "Please try different search criteria." : "Lütfen farklı arama kriterleri deneyiniz.";

    if (items.length === 0) {
        container.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 2rem;"><h3>${noResultsTitle}</h3><p>${noResultsDesc}</p></div>`;
        return;
    }

    // Button Text
    const detailBtnText = isEnglish ? "View Details" : "Detayları Gör";

    items.forEach(item => {
        const card = document.createElement('div');
        card.className = 'listing-card animate-up';

        // Data to display
        const displayTitle = isEnglish ? (item.title_en || item.title) : item.title;
        const displayLocation = isEnglish ? (item.location_en || item.location) : item.location;
        const displayStatus = isEnglish ? (item.status_en || item.status) : item.status;

        const badgeColor = item.status === 'Kiralık' ? 'rent' : '';

        card.innerHTML = `
            <div class="card-image">
                <img src="${item.coverImage}" alt="${displayTitle}">
                <span class="tag ${badgeColor}">${displayStatus}</span>
                <span class="price-tag ${badgeColor}">${item.price}</span>
                <div class="card-overlay">
                    <button class="view-btn" data-id="${item.id}">${detailBtnText}</button>
                </div>
            </div>
            <div class="card-content">
                <div class="location">
                    <i class="fas fa-map-marker-alt"></i> ${displayLocation}
                </div>
                <h3 class="card-title">${displayTitle}</h3>
                <div class="features">
                    <span><i class="fas fa-bed"></i> ${item.rooms}</span>
                    <span><i class="fas fa-ruler-combined"></i> ${item.area}</span>
                    <span><i class="fas fa-bath"></i> ${item.bath}</span>
                </div>
            </div>
        `;
        container.appendChild(card);

        const viewBtn = card.querySelector('.view-btn');
        viewBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            openModal(item);
        });

        card.addEventListener('click', () => {
            openModal(item);
        });
    });
}

function openModal(item) {
    if (!modal) return;

    const displayTitle = isEnglish ? (item.title_en || item.title) : item.title;
    const displayLocation = isEnglish ? (item.location_en || item.location) : item.location;
    const displayDescription = isEnglish ? (item.description_en || item.description) : item.description;
    const displayDetails = isEnglish ? (item.details_en || item.details) : item.details;
    const contactBtnText = isEnglish ? "Contact Agent" : "İletişime Geç";

    modalTitle.textContent = displayTitle;
    modalLocation.innerHTML = `<i class="fas fa-map-marker-alt"></i> ${displayLocation}`;
    modalPrice.textContent = item.price;
    modalDescription.textContent = displayDescription;

    // Set Main Image
    modalMainImage.src = item.images[0];

    // Generate Thumbnails
    modalThumbnails.innerHTML = '';
    item.images.forEach((imgSrc, index) => {
        const thumb = document.createElement('img');
        thumb.src = imgSrc;
        thumb.className = index === 0 ? 'thumb active' : 'thumb';
        thumb.onclick = () => {
            modalMainImage.src = imgSrc;
            document.querySelectorAll('.thumb').forEach(t => t.classList.remove('active'));
            thumb.classList.add('active');
        };
        modalThumbnails.appendChild(thumb);
    });

    // Generate Details Grid
    modalDetailsGrid.innerHTML = '';
    for (const [key, value] of Object.entries(displayDetails)) {
        const detailDiv = document.createElement('div');
        detailDiv.className = 'detail-item';
        detailDiv.innerHTML = `
            <span class="detail-label">${key}</span>
            <span class="detail-value">${value}</span>
        `;
        modalDetailsGrid.appendChild(detailDiv);
    }

    // Update Contact Button Text in Modal if necessary (usually only icon or icon + text)
    // The button is static HTML in portfolio.html, I should select it and update text
    const contactBtn = modal.querySelector('.contact-agent-btn a');
    if (contactBtn) {
        contactBtn.innerHTML = `<i class="fab fa-whatsapp"></i> ${contactBtnText}`;
    }

    modal.classList.add('show');
    document.body.style.overflow = 'hidden';
}

function closeListingModal() {
    modal.classList.remove('show');
    document.body.style.overflow = '';
}

if (closeModal) {
    closeModal.addEventListener('click', closeListingModal);
}

window.addEventListener('click', (e) => {
    if (e.target == modal) {
        closeListingModal();
    }
});

// Helper function
function normalizeString(str) {
    return str.toLocaleLowerCase('tr-TR')
        .replace(/ğ/g, 'g')
        .replace(/ü/g, 'u')
        .replace(/ş/g, 's')
        .replace(/ı/g, 'i')
        .replace(/ö/g, 'o')
        .replace(/ç/g, 'c');
}

function filterItems() {
    const rawSearchTerm = searchInput ? searchInput.value : '';
    const normalizedSearchTerm = normalizeString(rawSearchTerm);
    const selectedType = typeFilter ? typeFilter.value : '';

    const filtered = portfolioItems.filter(item => {
        // Search in both TR and EN titles/locations
        const titleTR = normalizeString(item.title);
        const locTR = normalizeString(item.location);
        const titleEN = normalizeString(item.title_en || "");
        const locEN = normalizeString(item.location_en || "");

        const matchesSearch = titleTR.includes(normalizedSearchTerm) ||
            locTR.includes(normalizedSearchTerm) ||
            titleEN.includes(normalizedSearchTerm) ||
            locEN.includes(normalizedSearchTerm);

        const matchesType = selectedType === '' || item.type === selectedType;

        return matchesSearch && matchesType;
    });

    renderPortfolio(filtered);
}

if (searchBtn) {
    searchBtn.addEventListener('click', filterItems);
}

if (searchInput) {
    searchInput.addEventListener('keyup', (e) => {
        if (e.key === 'Enter') {
            filterItems();
        } else {
            filterItems();
        }
    });
}

if (typeFilter) {
    typeFilter.addEventListener('change', filterItems);
}

document.addEventListener('DOMContentLoaded', () => {
    const urlParams = new URLSearchParams(window.location.search);
    const query = urlParams.get('q');

    if (query && searchInput) {
        searchInput.value = query;
    }

    filterItems();
});
